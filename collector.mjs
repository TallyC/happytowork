...
async function queryRoute(date,q,direction){const start=direction==='outbound'?q.from:q.to,end=direction==='outbound'?q.to:q.from;const window=direction==='outbound'?q.outboundWindow:q.inboundWindow;return visit('https://www.railway.gov.tw/tra-tip-web/tip/tip001/tip112/gobytime',async page=>{
 ...
 if(date===dates[0]){const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Taipei',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(new Date());const minutes=Number(parts.find(x=>x.type==='hour')?.value||0)*60+Number(parts.find(x=>x.type==='minute')?.value||0);const live=rows.filter(t=>{const dep=timeMinutes(t.departure),arr=timeMinutes(t.arrival);return dep<=minutes&&minutes<arr});const near=rows.filter(t=>{const dep=timeMinutes(t.departure);return dep>=minutes&&dep<=minutes+120});const priority=[...live,...near].filter((t,i,a)=>a.findIndex(x=>x.no===t.no)===i).slice(0,4);for(const train of priority){try{const detail=await queryTrainStatus(date,train.no,stationName(end));train.status=detail.status;train.lastReported=detail.lastReported;train.milestones=detail.stations;train.statusCheckedAt=new Date().toISOString()}catch(e){data.errors.push(`${date} ${train.no} 動態：${e.message}`)}}}
...
const timeMinutes=s=>{const m=String(s||'').match(/(\d{2}):(\d{2})/);return m?Number(m[1])*60+Number(m[2]):-1};
async function queryTrainStatus(date,no,target){return visit(`https://www.railway.gov.tw/tra-tip-web/tip/tip001/tip112/querybytrainno?rideDate=${date.replaceAll('-','/')}&trainNo=${encodeURIComponent(no)}`,async page=>{await page.locator('table').nth(1).waitFor({timeout:20000});const rows=await page.locator('table').nth(1).locator('tr').evaluateAll(rs=>rs.slice(1).map(row=>Array.from(row.querySelectorAll('td')).map(c=>c.innerText.replace(/\s+/g,' ').trim())));const row=rows.find(r=>r[0]===target);if(!row)throw Error(`找不到${target}站`);const allStations=rows.filter(r=>r[0]&&/^\d{2}:\d{2}$/.test(r[1]||'')).map(r=>({name:r[0].replace('台北','臺北').replace('台東','臺東'),arrival:r[1]||null,departure:r[2]||null,status:r.slice(3).find(v=>/準點|誤點|停駛|取消|運轉|到站|發車/.test(v))||null}));const reported=allStations.filter(r=>r.status);const lastReported=reported.at(-1)||null;const stations=allStations.filter(r=>['臺東','花蓮','羅東','宜蘭','臺北'].includes(r.name));return {status:row.slice(3).find(v=>/準點|誤點|停駛|取消|運轉|到站|發車/.test(v))||null,lastReported,stations,allStations}})}
try{
 ...
 const result=await queryTrainStatus(date,c.no,c.to);const from=result.allStations.find(s=>s.name===c.from),to=result.allStations.find(s=>s.name===c.to);if(!from||!to||!/^\d{2}:\d{2}$/.test(from.departure)||!/^\d{2}:\d{2}$/.test(to.arrival))throw Error('車站或時刻欄位缺漏');trains.push({...c,departure:from.departure,arrival:to.arrival,status:result.status,lastReported:result.lastReported,milestones:result.allStations,statusCheckedAt:new Date().toISOString(),url});
 ...
 const fields={};for(const row of rows){const label=norm(row.cells[0]?.innerText);if(!['溫度','降雨機率','天氣狀況','蒲福風級'].includes(label))continue;const vals=[];for(const cell of Array.from(row.cells).slice(1)){const value=label==='溫度'?norm(cell.querySelector('.tem-C')?.textContent):label==='天氣狀況'?(cell.querySelector('img')?.alt||norm(cell.innerText)).replace(/^天氣圖示，?/, ''):norm(cell.innerText);for(let i=0;i<(cell.colSpan||1);i++)vals.push(value)}fields[label]=vals}
